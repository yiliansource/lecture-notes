\begin{theorem} \label{thm:invertierbar-gdw-nichtverschwindend-multiplikativ}
    Sei $\Aa$ eine kommutative Banachalgebra mit Eins und $a \in \Aa$. Dann gilt $a \in \Inv(\Aa)$ genau dann, wenn $m(a) \neq 0$ für alle $m \in M_\Aa$, wobei $M_\Aa$ die Menge aller multiplikativen linearen Funktionale von $\Aa$ nach $\C$ bezeichnet, welche nicht identisch $0$ sind.
\end{theorem}
...
\begin{proof}
    ...
    Ist $a$ nicht invertierbar, so ist $a \cdot \Aa$ ein echtes Ideal in $\Aa$. Sei $I \supseteq a \cdot \Aa$ ein maximales Ideal. Dann gilt $U_1^\Aa(e) \subseteq \Inv(\Aa) \subseteq (I^c)^\circ = (\cl I)^c$ und somit $\cl I \neq \Aa$. Damit ist $I = \cl I$ und insbesondere ist $I$ abgeschlossen. Der Raum $\Aa / I$ ist mit der Quotientennorm ein Banachraum, und wir können durch $[c] \cdot [d] \coloneqq [cd]$ eine Multiplikation wohldefinieren, mit der $\Aa / I$ zu einer kommutativen Banachalgebra mit Eins (nämlich $[e]$) wird. Sei nun $[b] \in \Aa / I, b \neq [0]$. Da $I$ maximal ist, ist $\Aa / I$ ein Körper, womit $[b]$ invertierbar ist. Da also alle Elemente der Banachalgebra $\Aa / I$ außer Null invertierbar sind, liefert der Satz von Gelfand--Mazur einen isometrischen Isomorphismus
...
\begin{remark} \label{rem:gelfandraum-beschränkt}
...
\begin{lemma} \label{lem:funktional-reell-auf-selbstadjungierten}
...
\begin{corollary} \label{cor:selbstadjungiert-reelles-spektrum}
    Sei $\Aa$ eine $C^*$-Algebra mit Eins und $a \in \Aa$ mit $a = a^*$. Dann ist $\sigma_\Aa(a) \subseteq \R$.
\end{corollary}

\begin{proof}
    % cite
    Sei $\lambda \in \sigma_\Aa(a)$ und setze $\Ba \coloneqq [\{ e, a \}]_\Aa$, so ist $\Ba$ eine kommutative Unteralgebra von $\Aa$. Nach Bemerkung \ref{rem:eigenschaften-geschachtelter-cstern-algebren} (4) gilt $\sigma_\Aa(a) \subseteq \sigma_\Ba(a)$, also ist $a - \lambda e \notin \Inv(\Ba)$. Nach Satz \ref{thm:invertierbar-gdw-nichtverschwindend-multiplikativ} gibt es ein $m \in M_\Ba$ mit $m(a - \lambda e) = 0$, womit $\lambda = m(a)$ folgt, was nach Lemma \ref{lem:funktional-reell-auf-selbstadjungierten} reell ist.
\end{proof}

\begin{theorem}
    Sei $\Aa$ eine $C^*$ Algebra mit Eins $e$, und $\Ba \subseteq \Aa$ eine $C^*$-Unteralgebra, sodass $e \in \Ba$. Dann gilt
    \begin{itemize}
...
\begin{proof}
    Sei $b \in \Ba$ mit $b = b^*$. Dann gilt zunächst $ \sigma_\Aa(b) \subseteq \sigma_\Ba(b) \subseteq \R$ und damit $ \C \setminus \R \subseteq \rho_\Ba(b) \subseteq \rho_\Aa(b) $.
    Damit folgt
    ...
    Die zweite Aussage folgt unmittelbar aus der Ersten, wie in Bemerkung \ref{rem:eigenschaften-geschachtelter-cstern-algebren}.
\end{proof}

\begin{remark} {\ }
    \begin{itemize}
        \item Sei $\Aa$ eine Banachalgebra mit Eins. In Bemerkung \ref{rem:gelfandraum-beschränkt} haben wir $M_\Aa \subseteq K_1^{\Aa'}(0)$ gezeigt. Weiters kann man zeigen, dass $M_\Aa$ versehen mit der $w^*$-Topologie sogar ein kompakter Hausdorffraum ist.
        ...
        ist ein Banachalgebren-Homomorphismus mit $\Vert \hat{\cdot} \Vert = 1$.

        Für $a \in \Aa$ gilt $\hat{a}(M_\Aa) = \sigma_\Aa(a)$, da: Es gilt $\lambda \in \sigma_\Aa(a)$ genau dann, wenn $a - \lambda e \notin \Inv(\Aa)$, was genau dann der Fall ist wenn es ein $m \in M_\Aa$ mit $m(a - \lambda e) = 0$ gibt, was zu $\hat{a}(m) = m(a) = \lambda$ äquivalent ist, vgl. den Beweis von Korollar \ref{cor:selbstadjungiert-reelles-spektrum}.
    \end{itemize}
\end{remark}
...
\begin{proof}
    ...
    wobei die letzte Gleichheit gilt da alle Elemente von $\Aa$ normal sind.
...
\begin{definition} \label{def:phi-x}
    Sei $\Aa$ eine $C^*$-Algebra mit Eins und $x \in \Aa$ mit $x x^* = x^* x$. Dann ist
    $$ \Ba \coloneqq \clspn \{ x^j (x^*)^k \mid j, k \in \N_0 \} = [\{ e, x \}]_\Aa \subseteq \Aa $$
...
\begin{proof}
    ...
    auch $m_1((x^*)^k) = m_2((x^*)^k)$ für alle $k \in \N_0$. Damit haben wir Gleichheit auf dem Erzeuger, und damit auf dem gesamten Raum, also folgt $m_1 = m_2$.

    Da $\phi_x$ eine stetige, bijektive Abbildung zwischen kompakten Hausdorffräumen ist, folgt auch die Stetigkeit der Umkehrabbildung. Insgesamt ist $\phi_x$ also ein Homöomorphismus.
\end{proof}