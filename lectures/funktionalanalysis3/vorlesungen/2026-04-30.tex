...
\begin{lemma}
    ...
    \]
    ist wohldefiniert, bijektiv, verträglich mit $\leq$ und es gilt
    $$ \varepsilon^{-1}(q) = q(e - q)^{-1}, \quad q \in \Aa_+ \cap B_1^\Aa(0). $$
\end{lemma}
...
\begin{proof}
    ...
    Setze nun
    $$ \psi : \Aa_+ \cap B_1^\Aa(0) \to \eAa, \ q \mapsto q (e-a)^{-1}, $$
    ...
    Daraus folgt
    \[
        \varepsilon(p) = 0 \leq e - (e + p)^{-1} \leq e - (e + q)^{-1} = \varepsilon(q). \qedhere
    \]
\end{proof}