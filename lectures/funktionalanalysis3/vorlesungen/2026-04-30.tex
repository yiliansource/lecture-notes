...
\begin{proof}
    ...
    \[ 
        \Vert \varphi(a) \Vert^2 = \Vert \varphi(a^* a) \Vert = r(\varphi(a^* a)) \leq r(a^* a) = \Vert a^* a \Vert = \Vert a \Vert^2
    \]
...
\begin{proof}
    Wir betrachten $\Aa \vartriangleleft \eAa$. Es ist $\eAa \times \C$ auch eine $C^*$-Algebra (vgl. Übung), wobei die Operationen punktweise definiert sind, mit $(e, 1)$ als Eins. Jetzt gilt
    ...
    Betrachte $ (\Aa \times \{ 0 \}) \overset{\cdot}{+} \spn \{ (e, 1) \} \geq \Aa \times \{ 0 \}$, so gilt wieder Analoges für $\Ba$, wir nennen die linken Seiten $\Ca_1, \Ca_2$. Wir definieren nun
    $$ \widetilde{\varphi} : \Ca_1 \to \Ca_2, \ (a, 0) + \alpha (e, 1) \mapsto (\varphi(a), 0) + \alpha(e, 1), $$
    so ist diese Abbildung ein injektiver $\ast$-Algebren-Homomorphismus, mit
    \[
        \widetilde{\varphi}((e, 1)) = (e, 1).
    \]
