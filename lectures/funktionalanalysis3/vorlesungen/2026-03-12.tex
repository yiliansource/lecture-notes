\begin{definition}
    Seien $\Aa, x \in \Aa, \Ba$ wie in Definition \ref{def:phi-x}. Dann definieren wir
    \begin{equation*}
...
\begin{remark}{\ }
    \begin{enumerate}
        ...
        $f(x) \in [\{ e, x \}]_\Aa \subseteq \Aa$.

        \item Ist $f \in C_b(\sigma(x), \C), a \in \Aa, ax = xa, ax^* = x^* a$, so ist auch $af(x) = f(x) a$.
        
        \item Ist $f \in C_b(\sigma(x), \C)$, so ist $f(x)$ das eindeutige Element von $[\{e, x\}]$ mit der Eigenschaft $\widehat{f(x)} = \phi_x^t(f)$, beziehungsweise $\widehat{f(x)}(m) = \phi_x^t(f)(m)$ für alle $m \in M_{[\{e,x\}]}$. Inbesondere gilt
        $$ m(f(x)) = \phi_x^t(f)(m) = (f \circ \phi_x)(m) = f(m(x)). $$

        \item Es gilt, für $\Ba \coloneqq [\{e, x\}]$,
        $$ \sigma_\Ba (f(x)) = \{ m(f(x)) : m \in M_\Ba \} = f(\{ m(x) : m \in M_\Ba \}) = f(\sigma_\Ba (x)). $$
    \end{enumerate}
\end{remark}

\begin{proposition} \label{prop:komposition-des-kalküls}
...
\begin{proof}
    Es gilt $f(x) \in [\{e, x\}]_\Aa$ und damit $[\{ e, f(x) \}]_\Aa \leq [\{ e, x \}]_\Aa \leq \Aa$. Sei nun $m \in M_{[\{e,x\}]}$, so gilt:
    \begin{align*}
...
\begin{proposition} \label{prop:kalkül-erhält-normal}
...
Proposition \ref{prop:kalkül-erhält-normal} folgt aus Proposition \ref{prop:komposition-des-kalküls} mit $f = \iota$.

\begin{corollary}
    Mit den Voraussetzungen von Proposition \ref{prop:kalkül-erhält-normal} gilt, dass wenn $\varphi$ injektiv ist, $\varphi$ sogar isometrisch ist.
\end{corollary}
...
\begin{proof}
    ...
    Inbesondere folgt $\sigma_\Ba (\varphi(a^* a)) \subsetneq \sigma_\Aa(a^* a)$. Sei nun $f \in C_b(\sigma(a^* a), \C)$ mit $f \neq 0$ und $\restr{f}{\sigma_\Ba (\varphi(a^* a))} = 0$. Damit erhalten wir aus der vorigen Proposition
...
\begin{remark}
    ...
    \begin{enumerate}
        ...
        \item Es gilt $\iota_{\sigma(a)} \in C_b(\sigma(a), \C)$ und $\Vert \iota_{\sigma(a)} \Vert_\infty = \Vert a \Vert$. Damit gilt $0 \leq \max(0, \cdot) \leq \vert \iota_{\sigma(a)} \vert$ und
        ...
        $$ \Vert (\Vert p \Vert + \Vert q \Vert) e - (p+q) \Vert \leq \Vert\, \Vert p \Vert e - p \Vert + \Vert \, \Vert q \Vert e - q \Vert \leq \Vert p \Vert + \Vert q \Vert, $$
