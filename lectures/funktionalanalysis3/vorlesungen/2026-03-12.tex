...
\begin{remark}{\ }
    \begin{enumerate}
        ...
        \item Ist $f \in C_b(\sigma(x), \C)$, so ist $f(x)$ das eindeutige Element von $[\{e, x\}]$ mit der Eigenschaft $\widehat{f(x)} = \phi_x^t(f)$, beziehungsweise $\hat{f}(x)(m) = \phi_x^t(f)(m)$ für alle $m \in M_{[\{e,x\}]}$. Inbesondere gilt
